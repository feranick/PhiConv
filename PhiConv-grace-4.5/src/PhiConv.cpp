...
char version[]= "4.5-grace - 20100519";
...
{	
	...
	if(!infile2)
		...
		cout<<"\nIs Grace installed? (1: Yes,  other: no)   ";
		vplot0=ReadKey();
		
		if(vplot !=0)
			{cout<<"\nGrace (version 5.x) needs to be installed to be able to plot the ASCII files.\n";
			cout<<"Grace (freeware) can be downloaded at \n	  http://plasma-gate.weizmann.ac.il/Grace \n\n";}
	...
	typeAES=1;
	strncpy(difextension, ascextensionB, 5);	
...
int DnD(char *name)
	...
	tmp[strlen(tmp) - 4] = '\0';
	snprintf(tmp2, CMAXI, "%s%s", tmp, infoextension);
...
acq extract(char* name, char* nameout, char *nameinfo)
	...
	snprintf(nameOUT, CMAXI, "%s%s", nameout, datextension);	
	...
	if (Comp==0)
		{infile>>countArray[0];}
	if(Comp!=0)
		{snprintf(countArray[0], 2, "%s", "   ");
		//	strncpy(countArray[0],"  ",2);
		}
	...
	for(j=1; j<steps+1; j++)
		...
		if (Comp==0)
			{infile>>countArray[j];}
		if(Comp!=0)
			{snprintf(countArray[j], 2, "%s", "   ");
				//strncpy(countArray[j],"  ",2);
			}
		if (Comp2==0)
			{steps=j-1;}
		}
...
acq extractW32(char* name, char* nameout, char *nameinfo)
		...
		if (strcmp(test,"Comment")==0)
			...
			while(strcmp(test, "AcqMode")!=0)
				...
				if(strcmp(test, "AcqMode")!=0)
					{strncat(a.comment, test, CMAXI-1);
					strncat(a.comment, " ", 1);}
	...
	for(j=0; j<steps+1; j++)
		...
		//GracePrintf("g0.s0 point %f, %f", energy[j], Array[j]);	
...
int single()
	{	
		...
		snprintf(nameinfo, CMAXI, "%s%s", nameIN, infoextension);
...
int batch()
	{	//static char nameIN[CMAXI], nameout[CMAXI], name[CMAXI], nametmp[CMAXI];
		...
		snprintf(nameinfo, CMAXI, "%s_%d_batch%s"  , nameIN, f1,infoextension);
		//for (f=f1; f<=f2; f++)
		while (flag!=0)
			{snprintf(nametmp, CMAXI,"%s%d", nameIN, f1); 
			snprintf(nameout, CMAXI,"%s%d"  , nameIN, f1); 			
...
{	
	GraceRegisterErrorFunction(my_error_function);
	if (GraceOpen(2048) == -1) {
	        fprintf(stderr, "Can't run Grace from Plot. \n");
	        exit(-1);
	  	  }
...
void formatRec(char *nameIN, char *name)	
	...
	snprintf(name, CMAXI, "%s%s", nameIN, ascextensionB);	
	ifstream infile;
	...
	if(!infile)
		{snprintf(name, CMAXI, "%s%s", nameIN, ascextensionS);
		typeAES=1;		
		}
	infile.close();
	...
	if(!infile)
		{snprintf(name, CMAXI, "%s%s", nameIN, difextensionB);
		typeAES=0;
		}
	infile.close();
	...
	if(!infile)
		{snprintf(name, CMAXI, "%s%s", nameIN, difextensionS);
		typeAES=0;
		}
	infile.close();
	}
